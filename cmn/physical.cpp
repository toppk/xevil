...
Boolean Physical::corporeal_attack(PhysicalP,int damage) {
...
Boolean Moving::corporeal_attack(PhysicalP killer,int damage) {
  ...
  return Physical::corporeal_attack(killer,damage);
}
...
Droplet::Droplet(const DropletContext &cx,DropletXdata &x_data,
  ...
  : Moving(cx.movingContext,x_data,w,l,raw_pos,
           (dirInitial == CO_air ? vel.get_dir() : dirInitial)) {
  // If True, the dir never changes for the droplet.
  staticDir = (dirInitial != CO_air);
  inAir = True;
...
Size Droplet::get_size_max(DropletType dType) {
  // Of course the whole point of OO programming is to avoid switch statements 
  // like these.  Can't do that with static fntns though.
  switch (dType) {
  case DROPLET_BLOOD:
    return Blood::get_size_max();
  case DROPLET_GREEN_BLOOD:
    return GreenBlood::get_size_max();
  case DROPLET_OIL:
    return OilDroplet::get_size_max();
//  case DROPLET_SPARK:
//    return Spark::get_size_max();
  default:
    assert (0);
  }
  // Make compiler happy.
...
PhysicalP Droplet::create_and_add_if(DropletType dType,WorldP w,
                                     LocatorP l,const Pos &p,const Vel &v) {
  switch (dType) {
  case DROPLET_BLOOD:
    return Blood::create_and_add_if(w,l,p,v);
  case DROPLET_GREEN_BLOOD:
    return GreenBlood::create_and_add_if(w,l,p,v);
  case DROPLET_OIL:
    return OilDroplet::create_and_add_if(w,l,p,v);
//  case DROPLET_SPARK:
//    return Spark::create_and_add_if(w,l,p,v);
  default:
    assert (0);
  }
  return NULL;
...
void Droplet::set_entry(int index,const Id &id) {
  assert(index >=0 && index < dropletMax);
  dropletList[index] = id;
}
...
void Creature::drop_all() {
  for (int n = 0; n < AB_MAX; n++) {
    if (abilities[n]) {
      abilities[n]->drop_all();
    }
  }
...
Boolean Creature::corporeal_attack(PhysicalP killer,int damage) {
  ...
  if (Moving::corporeal_attack(killer,damage)) {
    // Don't create blood if attack is blocked.
    
    if (cc->dropletType != DROPLET_NONE) {
      // Create Blood drop.
      LocatorP locator = get_locator();
      const Area &area = get_area();
      Pos middle = area.get_middle();
      Size bloodSize = Droplet::get_size_max(cc->dropletType);
      ...
      for (int n = 0; n < compute_droplets_num(damage); n++) {
        ...
        Vel bloodVel((float)(Utils::choose(2 * SPLATTER_SPEED + 1) - SPLATTER_SPEED),
		     (float)-(Utils::choose(SPLATTER_SPEED)+1)); 

        // Try to create blood drop, don't care about return value.
        Droplet::create_and_add_if(cc->dropletType,get_world(),
				   locator,rawPos,bloodVel);
      }
    }
    return True;
  }
...
//    cre->set_vel_next(0);
  ...
  // Sliding along the ground to attack.
...
void Flying::act() {
  ...
  /* Deal with gravity here instead of in Creature because falling less than
     one pixel per clock. */
  cre->set_grav_next(0); // Don't use Creature's notion of gravity.
  ...
  case IT_R: 
    DO_INCREASE(velNext.dx);
    velNext = velNext + acceleration * unitAccs[CO_R];
    break;
  
  case IT_DN_R:
    DO_INCREASE(velNext.dx);
    DO_INCREASE(velNext.dy);
    if (stance == CO_air) {
      velNext = velNext + acceleration * unitAccs[CO_DN_R];
    }
    else {
      velNext = velNext + acceleration * unitAccs[CO_center_R];
    }
    break;
  ...
  case IT_DN:
    if (stance == CO_air) {
      DO_INCREASE(velNext.dy);
      velNext = velNext + acceleration * unitAccs[CO_DN];
    }
    else {
      velNext.set_zero();
    }
  ...
  case IT_DN_L:
    DO_DECREASE(velNext.dx);
    DO_INCREASE(velNext.dy);
    if (stance == CO_air) {
      velNext = velNext + acceleration * unitAccs[CO_DN_L];
    }
    else {
      velNext = velNext + acceleration * unitAccs[CO_center_L];
    }
    break;
  
  case IT_L:
    DO_DECREASE(velNext.dx);
    velNext = velNext + acceleration * unitAccs[CO_L];
    break;
  
  case IT_UP_L:
    if (stance == CO_air) {
      DO_DECREASE(velNext.dx);
      DO_DECREASE(velNext.dy);
      velNext = velNext + acceleration * unitAccs[CO_air_L];
    }
    else {
      DO_DECREASE(velNext.dx);
	    velNext = velNext + jump * unitAccs[CO_air_L];
	    cre->set_stance_next(CO_air);
    }
    break;
  ...
  case IT_UP:
    ...
    if (stance == CO_air) {
      DO_DECREASE(velNext.dy);
      velNext = velNext + acceleration * unitAccs[CO_air_UP];
    }
    else {
	    velNext = velNext + jump * unitAccs[CO_air_UP];
	    cre->set_stance_next(CO_air);
    }
    break;
  ...
  case IT_UP_R:
    if (stance == CO_air) {
      DO_INCREASE(velNext.dx);
      DO_DECREASE(velNext.dy);
      velNext = velNext + acceleration * unitAccs[CO_air_R];
    }
    else {
      DO_INCREASE(velNext.dx);
      velNext = velNext + jump * unitAccs[CO_air_R];
      cre->set_stance_next(CO_air);
    }
    break;
  };
