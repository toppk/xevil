...
// Defines
#define VERSION "2.02r2"
// 2.0 Beta 4B is the RedHat Linux version.
// 2.02 is GPL
// 2.02 r2 is updated for gcc 3.2
...
DifficultyLevel Game::difficultyLevels[DIFFICULTY_LEVELS_NUM] = {
  /* reflexes, enemiesInitial, enemiesIncr, enemiesMax, name */
  {  40,       2,              1,           10,         "trivial" },
  {  6, /*8*/  3,              2,           50,         "normal" },
  {  4,        5,              5,           50,         "hard" },
  {  0,        20,             10,          1000000,    "bend-over" },
};
...
void Game::post_clock() {
  ...
  // We need to do something cleaner, maybe two switches, one for Client,
  // one for StandAlone and Server.
  ...
  if (!pause) {
    ...
    // The big-ass state machine.  Maybe we should have a IGameState inteface with separate 
...
void Game::game_over_check(int humansPlaying,int enemiesPlaying) {
  ...
  if (style->game_over_check(hNum,enemiesPlaying)) {
    state = gameExtra;
...
void Game::new_level() {
  ...
  world.set_special_map_next(World::MAP_NONE);
  world.set_rooms_next(worldRooms);
...
//  assert(humansNum <= Xvars::HUMAN_COLORS_NUM);  no longer true.
  ...
  if (cooperative) {
    ...
    locator.add_persistent_team(humans_team,NULL);
  }
  else {
    locator.add_persistent_team(one_human_team,NULL);
  }
  ...
  switch (Utils::choose(8)) {
    ...
    case 4: { // drop weights 
      ...
      for (int x = WSQUARE_WIDTH * 1;
        ...
        locator.add(p);
        locator.add(p2);
        // moved to after the Locator::add(), hardts
        ((ItemP)p2)->use(NULL);
      }	    
      ...
      for (int m = 0; m < 10; m++) {
        ...
        Pos pos = world.empty_rect(Hero::get_size_max());
#if 0
        Size maxSize = Hero::get_size_max();
        Pos pos((titleOrigin.c + 1) * WSQUARE_WIDTH + 
                Utils::choose((W_TITLE_COL_MAX - 2) * WSQUARE_WIDTH 
                              - maxSize.width),
                titleOrigin.r * WSQUARE_HEIGHT + 
                W_TITLE_ROW_MAX * (WSQUARE_HEIGHT - 1) - maxSize.height);
#endif
        PhysicalP obj;
    ...
    case 6: { // pack o' dogs
      int n;
      // Game should probably have its own definition of dog team.
    ...
    case 7: {
      ...
      locator.add_team(universal_team,NULL);
...
Boolean Game::universal_team(LocatorP,PhysicalP,PhysicalP) {
  return True;
}
...
Boolean Game::humans_team(LocatorP l,PhysicalP p1,PhysicalP p2) {
  // Both are Human.
...
Boolean Game::one_human_team(LocatorP,PhysicalP p1,PhysicalP p2) {
  IntelP p1Intel = p1->get_intel();
  IntelP p2Intel = p2->get_intel();
  if (!p1Intel || !p2Intel) {
    return False;
