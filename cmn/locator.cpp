...
{
  //stupid MS compiler gets ambiguous call without the recast to double
  int glocRadius = (int)ceil((double)((double)radius / OL_GRID_SIZE_MIN)); 
...
TeamId Locator::add_team(Boolean (*same_team)(LocatorP,PhysicalP,PhysicalP),
                         TeamOptionsP ops) {
...
Boolean Locator::same_team(TeamOptions &ops,PhysicalP p1,PhysicalP p2) {
  ...
  // regular teams
  for (n = 0; n < teamsNum; n++) {
    if (teams[n].same_team(this,p1,p2)) {
  ...
  // persistent teams
  for (n = 0; n < persistentTeamsNum; n++) {
    if (persistentTeams[n].same_team(this,p1,p2)) {
      // Both are on team n.
