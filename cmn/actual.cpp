...
Blood::Blood(WorldP w,LocatorP l,const Pos &pos,const Vel &vel)
	 : Droplet(context,xdata,w,l,pos,vel)
{}
...
PhysicalP Blood::create_and_add_if(WorldP w,LocatorP l,const Pos &p,const Vel &v)
{ 
  PhysicalP blood = NULL;
  int index = get_available_entry(l);
  if (index != -1) {
    blood = new Blood(w,l,p,v);
    assert(blood);
    l->add(blood);
    set_entry(index,blood->get_id());  
  }
  return blood;
}
...
GreenBlood::GreenBlood(WorldP w,LocatorP l,const Pos &pos,const Vel &vel)
	 : Droplet(context,xdata,w,l,pos,vel)
{}
...
PhysicalP GreenBlood::create_and_add_if(WorldP w,LocatorP l,const Pos &p,const Vel &v)
{ 
  PhysicalP blood = NULL;
  int index = get_available_entry(l);
  if (index != -1) {
    blood = new GreenBlood(w,l,p,v);
    assert(blood);
    l->add(blood);
    set_entry(index,blood->get_id());  
  }
...
OilDroplet::OilDroplet(WorldP w,LocatorP l,const Pos &pos,const Vel &vel)
	 : Droplet(context,xdata,w,l,pos,vel)
{}
...
PhysicalP OilDroplet::create_and_add_if(WorldP w,LocatorP l,const Pos &p,const Vel &v)
{ 
  PhysicalP blood = NULL;
  int index = get_available_entry(l);
  if (index != -1) {
    blood = new OilDroplet(w,l,p,v);
    assert(blood);
    l->add(blood);
    set_entry(index,blood->get_id());  
  }
...
PetP DogWhistle::create_pet(IntelP master,Boolean alternatePet,int n) {
  ...
  IntelOptions ops;
  ops.psychotic = True;
  ops.classFriends = False; // So Dogs attack enemy dogs.
